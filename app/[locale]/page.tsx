...
export function generateMetadata({ params }: { params: { locale: Locale } }): Metadata {
  ...
  return {
    title: isEt
      ? 'Iluõis | Massaaž, teraapia, jooga ja hooldusteenused Tallinnas'
      : 'Iluõis | Massage, therapy, yoga and care services in Tallinn',
    description: isEt
  ...
  return (
    <PageShell locale={params.locale}>
      <section className="mx-auto grid min-h-screen max-w-7xl items-center gap-16 px-6 py-20 md:grid-cols-2 md:px-10">
        <Reveal>
          <p className="mb-6 text-xs uppercase tracking-widest text-ink">
            {t.heroKicker}
          </p>

          <h1 className="luxury-title mb-8 whitespace-pre-line text-6xl leading-tight tracking-tight md:text-8xl">
            {t.heroTitle}
          </h1>

          <p className="mb-10 max-w-xl text-lg leading-relaxed text-ink">
            {t.heroText}
          </p>
          ...
          <div className="flex flex-wrap gap-4">
            <Link
              ...
              className="rounded-full bg-roseGold px-8 py-4 text-sm uppercase tracking-widest text-white shadow-xl transition hover:scale-105"
            >
              {t.book}
            </Link>

            <Link
              href={`/${params.locale}/massaz`}
              className="rounded-full border border-[#e8c7cd] bg-white px-8 py-4 text-sm uppercase tracking-widest text-roseGoldDeep transition hover:bg-violet-50"
            >
              {t.readMore}
            </Link>
          </div>
        ...
        <div className="mx-auto max-w-7xl">
          <Reveal>
            <div className="mb-16 flex flex-col gap-8 md:flex-row md:items-end md:justify-between">
              <div>
                <p className="mb-4 text-xs uppercase tracking-widest text-roseGold">
                  {t.services}
                </p>

                <h2 className="rose-heading text-5xl leading-tight">
                  {t.servicesTitle}
                </h2>
              </div>
            ...
            {items.map((service, index) => (
              <Reveal key={service.slug} delay={index * 0.06}>
                ...
                >
                  ...
                  <h3 className="rose-heading mb-4 text-3xl leading-tight">
                    {service.title}
                  </h3>
                  ...
                  <span className="text-xs uppercase tracking-widest text-roseGoldDeep">
                    {t.readMore} →
                  </span>
                </Link>
              </Reveal>
          ...
          <Reveal delay={0.1}>
            <div className="soft-card rounded-organic p-10">
              {[
                ...
                'Rahulik ja esteetiline keskkond',
                '20+ aastat kogemust',
                'Keha ja meele tasakaal',
              ].map((item) => (
                <div
                  key={item}
